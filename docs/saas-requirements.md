...
| **RSK-03** | A client release moves the behaviour this design rests on | **H** | **H** | NFR-10 canary before adopting a release as the floor; NFR-22 isolates the tracking code. The canary must watch `x-stainless-package-version` and the dated `anthropic-beta` list as well as the CLI version — they move independently (ASM-40) | Detection is reactive — the canary runs after the release exists. **And a detected change may not be deployable without a restart until NFR-17.5 lands** (ASM-15) | Operator | Every client release |
