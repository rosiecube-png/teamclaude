...
| **ASM-30** | The certificate directory is written by one process | ❌ **unverified, and now sampled more often** — `ensureCerts` does an atomic write per file (`src/mitm.js`), so two writers can interleave a CA from one run with a leaf from another. The pair is now read on every intercepted CONNECT rather than once per process, so a mismatched pair is met sooner. It is self-healing — the reuse check reports `the leaf is not signed by the stored CA` and a consistent pair is written — but two processes racing can each regenerate. Concurrent CONNECTs *within* one process share a single in-flight check; nothing coordinates across processes. Owned by task-5 |
